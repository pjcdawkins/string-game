...
- **Bow** the string: drag sideways below the fingerboard. Stroke speed = bow
  speed; vertical position = contact point (*sul tasto ⇠⇢ sul ponticello*);
  bow force from the slider, or pen/touch pressure. Too little force whistles
  (surface sound), too much crunches (raucous regime) — exactly the
  Schelleng-diagram behaviour, and the HUD tells you whether the model is in
...
- **Bow junction**: the McIntyre–Schumacher–Woodhouse stick–slip friction
  ...
  (the B/C split) gives sul tasto/ponticello naturally; bow force and speed
  span surface-sound → Helmholtz → raucous regimes. A little force noise
  supplies the breath of the bow hair.
...
- bowing closer to the bridge brightens the spectrum;
